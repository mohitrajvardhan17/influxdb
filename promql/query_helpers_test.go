...
import (
	...
	flux_promql "github.com/influxdata/flux/promql"
...
func (q *testQuerier) runQueryVariants(query string, variantArgs []string, args map[string]string, skipComparison bool, shouldFail bool) {
	...
	if len(variantArgs) == 0 {
		...
		q.runQuery(query, skipComparison, shouldFail)
		return
	}
	...
	for _, vArg := range variantArgs {
		...
		for _, variantVal := range vals {
			...
			q.runQueryVariants(query, filteredVArgs, args, skipComparison, shouldFail)
		}
	}
}

func (q *testQuerier) runQuery(qry string, skipComparison bool, shouldFail bool) {
	// Transpile PromQL into Flux.
	promqlNode, err := promql.ParseExpr(qry)
	if err != nil {
		q.t.Fatalf("Error parsing PromQL expression %q: %s", qry, err)
	}
	tr := &flux_promql.Transpiler{
		Bucket:     q.influxDB.Bucket.Name,
		Start:      q.start,
		End:        q.end,
		Resolution: q.resolution,
	}
	fluxFile, trErr := tr.Transpile(promqlNode)
	...
	if (promResult.Err != nil) != shouldFail {
		...
		q.t.Fatalf("Expected Prometheus query %q to fail, but succeeded", qry)
	}
	...
	err = q.influxDB.QueryAndConsume(q.ctx, req, func(r flux.Result) error {
		influxMatrix, err = FluxResultToPromQLValue(r, promql.ValueTypeMatrix)
		return err
	})
	if (err != nil) != shouldFail {
		if err != nil {
			q.t.Fatalf("Error querying InfluxDB for %q: %s\n\nFlux script:\n\n%s", qry, err, ast.Format(fluxFile))
		}
		q.t.Fatalf("Expected Flux query for %q to fail, but succeeded. Flux script:\n\n%s\n\nOutput matrix: %s", qry, ast.Format(fluxFile), influxMatrix.String())
	}

	if skipComparison || shouldFail {
		return
	}
